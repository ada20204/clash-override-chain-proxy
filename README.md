...
借助机场 + 家宽 IP 的链式代理，把域外 AI 服务（Claude、ChatGPT、Gemini 等）的出口 IP 切换为家庭宽带住宅 IP，降低因配置失误、流量泄漏或错误地区回退带来的风险。当前实现支持你继续手动选择 `chainRegion`；在默认严格模式下，AI 及相关平台流量必须强制走你当前选择的那条链式代理，链路不成立就直接报错。
...
4. **构造严格 AI 代理组**——默认启用 `strictAiRouting`，把 AI 服务、登录/认证/下载/IDE 相关平台统一收口到内部代理组 **`AI 严格链式代理`**
5. **统一注入链式代理规则**——严格 AI 流量命中 `AI 严格链式代理`，浏览器与社交流媒体等非 AI 类别继续走现有链式代理逻辑
...
打开 [`src/家宽IP-链式代理.js`](src/%E5%AE%B6%E5%AE%BDIP-%E9%93%BE%E5%BC%8F%E4%BB%A3%E7%90%86.js)，顶部现在有五个参数可以改：

```javascript
var USER_OPTIONS = {
  chainRegion: "SG", // 链式代理地区（自选节点 + 家宽IP 出口）
  manualNode: "", // 手动指定跳板节点名（留空 = 自动选最快的）
  strictAiRouting: true, // 是否启用 AI 严格链式代理模式
  enableBrowserProcessProxy: false, // 是否把浏览器进程整体送入链式代理
  enableAiCliProcessProxy: true, // 是否把 AI CLI 可执行文件送入链式代理
};
```

- **`chainRegion`**（可选值：`US` `JP` `HK` `SG`）——链式代理流量从哪个地区出去；默认严格模式下，AI 服务、认证/下载/IDE 相关平台都会强制跟随这个参数
- **`manualNode`**（节点名 / 留空）——指定跳板节点；留空则自动选该地区延迟最低的线路
- **`strictAiRouting`**（`true` / `false`）——默认 `true`。开启后，AI 服务、支撑平台和验证域名会先进入 **`AI 严格链式代理`**，再唯一指向当前 `chainRegion` 对应的家宽出口；如果这条链路构造失败，脚本会直接报错。关闭后进入兼容模式：AI / 支撑平台规则仍然走当前 `chainRegion` 的家宽出口，但不再创建独立 strict 代理组，保证也相应变弱
...
> 如果 `chainRegion` 找不到可用的地区节点 / 代理组，`manualNode` 名称写错，或者严格 AI 代理组没有唯一指向你当前 `chainRegion` 对应的家宽出口，脚本会直接报错，不再静默退化成未绑定跳板的家宽出口、更不会偷偷回到直连或错误地区。
...
③ **🇸🇬|新加坡-链式代理-家宽IP出口**——当前 `chainRegion` 对应的家宽链式出口
④ **AI 严格链式代理**——默认 strict 模式下新增；AI 服务、基础平台、验证域名与相关 App / CLI 先命中这里，再唯一指向当前 `chainRegion` 对应的家宽出口

5. 验证是否生效：
   - [ping0.cc](https://ping0.cc/) 或 [ipinfo.io](https://ipinfo.io/)——应显示当前链式代理对应的住宅 IP，不是机房 IP
   - [claude.ai](https://claude.ai/) / ChatGPT macOS App——默认 strict 模式下，应命中 **`AI 严格链式代理`**，并最终只指向你当前 `chainRegion` 对应的 **`*-链式代理-家宽IP出口`**

---

## 分流一览

- **严格 AI 流量** → 默认命中 **`AI 严格链式代理`**：Claude、ChatGPT、Sora、Gemini、NotebookLM、Antigravity、Perplexity、OpenRouter、Grok / xAI，以及 Claude / ChatGPT / Perplexity / Cursor / Windsurf 等 macOS AI App
- **AI CLI** → 可选严格链式代理（默认开启）：`claude`、`opencode`、`gemini`、`codex`
- **支撑平台** → 默认命中 **`AI 严格链式代理`**：`google.com`、`googleapis.com`、`googleusercontent.com`、`gstatic.com`、`microsoft.com`、`live.com`、`windows.net`、`office.com`、`m365.cloud.microsoft`、`sharepoint.com`、`onedrive.com`、`microsoftonline.com`、GitHub、Visual Studio / VS Code 相关域名，以及 Google Drive、Teams、Outlook、Word、Excel、PowerPoint、OneDrive、VS Code
...
- **出口测试** → 默认命中 **`AI 严格链式代理`**：ping0.cc、ipinfo.io
- **兼容模式**（`strictAiRouting: false`）→ AI / 支撑平台规则仍跟随 `chainRegion` 的家宽出口，但不再创建 **`AI 严格链式代理`**，同时失去“严格组唯一收口”和对应的 fail-closed 保证
...
- `DIRECT` 保护规则优先置顶
- 默认 strict 模式下会创建 **`AI 严格链式代理`**，并且它只能指向当前 `chainRegion` 的家宽出口
- 兼容模式下不会创建 **`AI 严格链式代理`**，AI / 支撑平台规则会直接指向当前 `chainRegion` 的家宽出口
...
- **为什么现在多了 `AI 严格链式代理`？**：默认 strict 模式会先把 AI 服务、支撑平台和验证域名收口到这个内部代理组，再唯一指向你当前 `chainRegion` 的家宽出口。这样做是为了防止它们静默回退到直连、`节点选择` 或其它地区
...
- **`strictAiRouting: false` 是干什么的？**：这是兼容模式。它会保留当前 `chainRegion` 的家宽出口目标，但不再创建 **`AI 严格链式代理`**，也不再提供 strict 模式下那套“唯一收口 + fail closed”的保证。只有在你明确接受较弱保证时再关闭它
