...
4. **构造严格 AI 路由目标**——默认启用 `strictAiRouting`，把 AI 服务、登录/认证/下载/IDE 相关平台直接锁定到当前 `chainRegion` 对应的家宽出口
...
- **`strictAiRouting`**（`true` / `false`）——默认 `true`。开启后，AI 服务、支撑平台和验证域名会被规则直接锁定到当前 `chainRegion` 对应的家宽出口；如果这条链路构造失败，脚本会直接报错。关闭后进入兼容模式：AI / 支撑平台规则仍然走当前 `chainRegion` 的家宽出口，但不再启用这套额外的 strict 校验，保证也相应变弱
...
> 如果 `chainRegion` 找不到可用的地区节点 / 代理组，`manualNode` 名称写错，或者 strict 模式下 AI / 支撑平台规则没有直接锁定到你当前 `chainRegion` 对应的家宽出口，脚本会直接报错，不再静默退化成未绑定跳板的家宽出口、更不会偷偷回到直连或错误地区。
...
③ **🇸🇬|新加坡-链式代理-家宽IP出口**——当前 `chainRegion` 对应的家宽链式出口；默认 strict 模式下，AI 服务、基础平台、验证域名与相关 App / CLI 会被规则直接锁定到这里

5. 验证是否生效：
   - [ping0.cc](https://ping0.cc/) 或 [ipinfo.io](https://ipinfo.io/)——应显示当前链式代理对应的住宅 IP，不是机房 IP
   - [claude.ai](https://claude.ai/) / ChatGPT macOS App——默认 strict 模式下，应直接命中你当前 `chainRegion` 对应的 **`*-链式代理-家宽IP出口`**

---

## 分流一览

- **严格 AI 流量** → 默认直接命中当前 `chainRegion` 的家宽出口：Claude、ChatGPT、Sora、Gemini、NotebookLM、Antigravity、Perplexity、OpenRouter、Grok / xAI，以及 Claude / ChatGPT / Perplexity / Cursor / Windsurf 等 macOS AI App
- **AI CLI** → 可选严格链式代理（默认开启）：`claude`、`opencode`、`gemini`、`codex`
- **支撑平台** → 默认直接命中当前 `chainRegion` 的家宽出口：`google.com`、`googleapis.com`、`googleusercontent.com`、`gstatic.com`、`microsoft.com`、`live.com`、`windows.net`、`office.com`、`m365.cloud.microsoft`、`sharepoint.com`、`onedrive.com`、`microsoftonline.com`、GitHub、Visual Studio / VS Code 相关域名，以及 Google Drive、Teams、Outlook、Word、Excel、PowerPoint、OneDrive、VS Code
...
- **出口测试** → 默认直接命中当前 `chainRegion` 的家宽出口：ping0.cc、ipinfo.io
- **兼容模式**（`strictAiRouting: false`）→ AI / 支撑平台规则仍跟随 `chainRegion` 的家宽出口，但不再启用 strict 模式下那套额外的 direct-target 校验和 fail-closed 保证
...
- `DIRECT` 保护规则优先置顶
- 默认 strict 模式下，AI / 支撑平台规则会直接指向当前 `chainRegion` 的家宽出口，并清理遗留的显式 AI 代理组
- 兼容模式下，AI / 支撑平台规则仍会直接指向当前 `chainRegion` 的家宽出口，但不会跑 strict 模式下那套额外校验
...
- **为什么取消了显式 AI 代理组？**：当前实现已经把 strict 模式收敛成“AI / 支撑平台规则直接锁定当前 `chainRegion` 出口 + 额外 fail-closed 校验”。保留额外代理组只会重复表达同一个目标，所以这次把它取消了
...
- **`strictAiRouting: false` 是干什么的？**：这是兼容模式。它会保留当前 `chainRegion` 的家宽出口目标，但不再提供 strict 模式下那套“direct target 校验 + fail closed”的保证。只有在你明确接受较弱保证时再关闭它
